...
- (void)initialiseAllTitles {
    ...
    for (int i=0; i<self.indexItems.count; i++) {
        ...
        UILabel *itemLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.marginRight, startY, self.itemMaxSize.width + 4, self.itemMaxSize.height + 4)];
...
- (void)setSelectedIndex:(NSInteger)selectedIndex {
    ...
    if (newIndex >= 0 && newIndex < self.itemsViewArray.count) {
        ...
        if ((firstLetter >= 'a' && firstLetter <= 'z')
            ...
            self.selectedBackView.center = newItemLabel.center;
            self.selectedBackView.layer.cornerRadius = 6;
        }
...
- (UIView *)selectedBackView {
    if (!_selectedBackView) {
        ...
        _selectedBackView.backgroundColor = [UIColor colorWith8BitRed:0 green:138 blue:203];
